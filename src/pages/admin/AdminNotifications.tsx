...
import { Bell, Trash2, Mail, Phone, User, Users, Search, Loader2, ExternalLink, ChevronRight, X, Square, CheckSquare, Send } from "lucide-react";
...
export default function AdminNotifications() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirm("Are you sure you want to remove this notification? This action cannot be undone.")) return;
    ...
    try {
      ...
      // We don't call handleFirestoreError here to avoid throwing and potentially breaking the UI state
      // instead we just log it for debugging
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center text-left">
        <div>
          <h1 className="text-3xl font-black tracking-tighter text-slate-900 italic uppercase">Launch Notifications</h1>
      ...
      <div className="grid gap-6">
        <AnimatePresence mode="popLayout">
          {filtered.map((n, i) => (
            ...
            >
              <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 cursor-pointer">
                <div className="flex gap-6 items-start">
                  ...
                  <div className="h-16 w-16 bg-slate-100 rounded-2xl flex items-center justify-center shrink-0 group-hover:bg-blue-50 transition-colors">
                    <User className="h-8 w-8 text-slate-400 group-hover:text-blue-600" />
                  </div>
                  <div className="space-y-1 text-left">
                    <h3 className="text-xl font-black italic tracking-tighter text-slate-900 flex items-center gap-2">
                      {n.fullName}
                    ...
                    <div className="flex flex-wrap gap-4 text-sm font-bold text-slate-500">
                      <div className="flex items-center gap-1.5 hover:text-blue-600 cursor-copy">
                        <Mail className="h-4 w-4" /> {n.email}
                      </div>
                      <div className="flex items-center gap-1.5 hover:text-blue-600 cursor-copy">
                        <Phone className="h-4 w-4" /> {n.phone}
                      </div>
                      <div className="flex items-center gap-1.5 text-slate-400">
                        <Bell className="h-4 w-4" /> {n.createdAt ? format(n.createdAt.toDate(), 'MMM d, yyyy h:mm a') : 'Just now'}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {n.referrals && n.referrals.length > 0 && (
                <div className="bg-slate-50/50 border-t border-slate-100 p-6 md:p-8" onClick={(e) => e.stopPropagation()}>
                    ...
                    {n.referrals.map((ref, idx) => {
                      ...
                      return (
                        <div 
                          ...
                          className={`bg-white border rounded-xl p-4 flex items-center gap-3 shadow-sm transition-all cursor-pointer ${isRefSelected ? 'border-blue-400 bg-blue-50/50' : 'border-slate-200'}`}
                        >
                          <Checkbox 
                            checked={isRefSelected}
                            onCheckedChange={() => handleToggleReferralSelect(ref.email, ref.email, n.id)}
                            onClick={(e) => e.stopPropagation()}
