...
export default function BrokerageSettings() {
  ...
  const [softOverageModalTab, setSoftOverageModalTab] = useState<"email" | "audit" | "rules" | "history">("email");
          ...
          <DialogHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogTitle className="text-lg font-black text-slate-900 mt-0.5">
                  Soft Overages Policy Configuration & Dispatch Audit
                </DialogTitle>
                <DialogDescription className="text-xs text-slate-500">
                  Comprehensive audit trail, confirmation email receipt, and policy rules for plan administrators.
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          {softOverageEmailData && (
            <div className="space-y-4 pt-2">
              ...
              <div className="flex items-center gap-1.5 border-b border-slate-200 pb-2">
                <button
                  type="button"
                  onClick={() => setSoftOverageModalTab("email")}
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                    softOverageModalTab === "email"
                      ? "bg-blue-600 text-white shadow-xs"
                      : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                  }`}
                >
                ...
                <button
                  type="button"
                  onClick={() => setSoftOverageModalTab("audit")}
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                    softOverageModalTab === "audit"
                      ? "bg-blue-600 text-white shadow-xs"
                      : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                  }`}
                >
                ...
                <button
                  type="button"
                  onClick={() => setSoftOverageModalTab("rules")}
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                    softOverageModalTab === "rules"
                      ? "bg-blue-600 text-white shadow-xs"
                      : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                  }`}
                >
                ...
                <button
                  type="button"
                  onClick={() => setSoftOverageModalTab("history")}
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                    softOverageModalTab === "history"
                      ? "bg-blue-600 text-white shadow-xs"
                      : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                  }`}
                >
              ...
              {softOverageModalTab === "email" && (
                <div className="rounded-xl border border-slate-200 overflow-hidden bg-white shadow-xs">
                  <div className="bg-slate-100/80 px-4 py-2.5 border-b border-slate-200 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Mail className="h-4 w-4 text-blue-600" />
                      <span className="text-xs font-bold text-slate-800">Direct Email Notification Receipt</span>
                    </div>
                  ...
                  <div className="p-4 space-y-3 text-xs">
                    <div className="space-y-1 pb-2 border-b border-slate-100 text-slate-600">
                      ...
                      <div className="flex gap-2">
                        <strong className="text-slate-900 w-16 shrink-0">From:</strong>
                        <span className="font-mono text-slate-700">Platform Notifications &lt;notifications@aiopenhouseconnect.com&gt;</span>
                      </div>
                      <div className="flex gap-2">
                    ...
                    <div className="flex items-center justify-between pt-2 text-[11px] text-slate-500">
                      <span>Message ID: <code className="text-blue-600">{softOverageEmailData.trackingId}-smtp</code></span>
                      <a
                        href="/admin/emails"
                        className="text-blue-600 hover:text-blue-800 font-bold flex items-center gap-1 hover:underline cursor-pointer"
                      >
                        Open in Email Outbox Logs <ExternalLink className="h-3 w-3" />
                      </a>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-slate-900 text-slate-100 rounded-xl font-mono text-[11px] overflow-x-auto">
                    <div className="text-slate-400 pb-1 border-b border-slate-800 flex justify-between">
                      <span>// Firestore system_logs record</span>
                      <span className="text-emerald-400">ID: {softOverageEmailData.auditLogId}</span>
                    </div>
                    <pre className="pt-2 leading-relaxed text-slate-200">
{JSON.stringify(
  {
    log_id: softOverageEmailData.auditLogId,
    event_type: "ADMIN_POLICY_CHANGE",
    policy: "ALLOW_SOFT_OVERAGES",
    status: softOverageEmailData.action.toUpperCase(),
    brokerage: {
      name: softOverageEmailData.brokerageName,
      code: softOverageEmailData.brokerageCode,
      tier: "Starter Tier ($99/mo)"
    },
    plan_admin: {
      name: softOverageEmailData.recipientName,
      email: softOverageEmailData.recipient,
      email_dispatched: true,
      dispatch_timestamp: softOverageEmailData.sentAt
    },
    changed_by: softOverageEmailData.adminActor,
    client_ip: "192.168.1.104",
    compliance_verification: "VERIFIED_COMPLIANT"
  },
  null,
  2
)}
                  ...
                  <div className="flex items-center justify-between p-3 bg-blue-50/60 border border-blue-200 rounded-xl text-xs text-blue-900">
                    <span className="flex items-center gap-2">
                      <ShieldCheck className="h-4 w-4 text-blue-600 shrink-0" />
                      Permanent cryptographic tamper-resistant record written to system audit trail.
                    </span>
                    <a
                      href="/admin/logs"
                      className="bg-blue-600 text-white font-bold px-3 py-1.5 rounded-lg hover:bg-blue-700 transition flex items-center gap-1 cursor-pointer shrink-0 text-[11px]"
                    >
                      Open Full Audit Logs <ExternalLink className="h-3 w-3" />
                    </a>
                  </div>
                </div>
              )}

              {/* TAB 3: POLICY & OVERAGES RULES */}
              {softOverageModalTab === "rules" && (
                <div className="space-y-3 text-xs">
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-2.5">
                    <h4 className="font-bold text-slate-900 flex items-center gap-2">
                      <Info className="h-4 w-4 text-blue-600" />
                      Soft Overages Policy & Pricing Architecture
                    </h4>
                    <p className="text-slate-600 leading-relaxed">
                      When <strong>Allow Soft Overages (Pay-per-Lead)</strong> is enabled, open house attendee kiosk check-ins and AI visitor captures will never be hard-blocked even if the monthly allowance (e.g. 500 leads on Starter Plan) is exceeded.
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1">
                      <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                        <span className="text-[10px] text-slate-400 font-bold uppercase block">Pay-per-Lead Rate</span>
                        <span className="text-sm font-black text-slate-900">$0.25</span>
                        <span className="text-[10px] text-slate-500 block">per incremental lead</span>
                      </div>
                      <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                        <span className="text-[10px] text-slate-400 font-bold uppercase block">Warning Threshold</span>
                        <span className="text-sm font-black text-amber-600">{selectedBrokerage?.quota_limits.warning_threshold_pct || 80}%</span>
                        <span className="text-[10px] text-slate-500 block">automated alert trigger</span>
                      </div>
                      <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                        <span className="text-[10px] text-slate-400 font-bold uppercase block">Cycle Reconciliation</span>
                        <span className="text-sm font-black text-blue-600">{formatCycleDate(selectedBrokerage?.quota_usage.cycle_end_date || "2026-08-31")}</span>
                        <span className="text-[10px] text-slate-500 block">invoice settlement date</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* TAB 4: CHANGE HISTORY & DUMMY RECORD */}
              {softOverageModalTab === "history" && (
                <div className="space-y-3">
                    ...
                    <table className="w-full text-left text-xs">
                      <thead className="bg-slate-100 text-slate-600 uppercase text-[10px] font-bold">
                        <tr>
                          ...
                          <th className="p-2.5">Admin Actor</th>
                      ...
                      <tbody className="divide-y divide-slate-100 bg-white">
                        <tr className="hover:bg-slate-50/80">
                          <td className="p-2.5 font-mono text-[11px] text-slate-700">Aug 19, 2026, 09:15 AM</td>
                          <td className="p-2.5">
                            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-200">
                              Enabled
                            </span>
                          </td>
                          <td className="p-2.5 text-slate-800 font-medium">Super Administrator</td>
                          <td className="p-2.5 font-mono text-[11px] text-blue-600">sarah.jenkins@centurypremier.com</td>
                          <td className="p-2.5">
                            <span className="text-emerald-600 font-bold text-[10px] flex items-center gap-1">
                              <CheckCircle className="h-3 w-3" /> Delivered
                            </span>
                          </td>
                          <td className="p-2.5 font-mono text-[10px] text-slate-500">OVRG-8F92K</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  <div className="text-[11px] text-slate-400 text-right">
                    Showing 1 verified policy audit record for {softOverageEmailData.brokerageName}.
                  </div>
                </div>
              )}
                ...
                <div className="flex items-center gap-2">
                  <Button
                    onClick={() => {
                      ...
                      setSoftOverageModalOpen(false);
                    }}
                    variant="outline"
                    className="text-slate-700 text-xs font-bold px-3 py-1.5 rounded-xl cursor-pointer"
                  >
